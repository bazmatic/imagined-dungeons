...
const SYSTEM_PROMPT = (npc: Agent): string => {
  ...
  lines.push(
    '3. Manage your own `Current short-term intent` (shown in the header above). Each turn, decide:\n   a. If you HAVE NO intent yet but the situation calls for one (someone asked you to do something multi-step, you decided to pursue a goal, or there is anything else worth doing here — even passive engagement like "watch what is happening" or "wait for Paff to leave before I act"), declare it with `INTENT: <full plan>`. Capture the WHOLE task ("deliver the fire map to Captain Serena near the docks"), not just the first step.\n   b. If you HAVE an intent and have just carried it out (cross-reference your witnessed events and current state), prefix with `INTENT_DONE`. If you can immediately see something else worth doing, also include `INTENT: <new plan>` on the next line — that keeps you in the scene. Without a follow-up intent, you will go dormant after this turn.\n   c. If you HAVE an intent and your understanding of it has sharpened (you learned the destination, the recipient, etc.), restate it with `INTENT: <refined plan>`.\n   d. If you HAVE an intent and it is still in progress, do not emit a control line — just take the next concrete step (pick up the item, move toward the destination, hand it over, etc.).\n   IMPORTANT: An empty `Current short-term intent` at the end of a turn means you go dormant (you stop ticking). If something interesting is happening that you want to be part of — a conversation, an unfolding situation, someone in danger — declare an intent for it. "I have no plan" is fine and goes dormant; "I have nothing to do but the scene matters" is wrong — declare an observational intent.',
  );
  lines.push(
