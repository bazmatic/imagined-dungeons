...
- speak: say something. Speech is broadcast — anyone present hears it. Set targetAgentRef only when the actor is unambiguously addressing one specific agent (e.g. "tell spark, hi", "say hi to the goblin"); otherwise leave targetAgentRef null and let listeners decide whether they were addressed.
  ...
  IMPORTANT: if the input is a bare conversational-initiation with no utterance content — "talk to X", "chat to X", "chat with X", "greet X", "speak to X", "approach X" — the actor wants to start a conversation but hasn't said what. Generate a brief, natural opening line appropriate to the setting and the named character. Never use the raw command text as the utterance (e.g. utterance must NOT be "talk to Smolder" or "chat to Uncle Bob").
