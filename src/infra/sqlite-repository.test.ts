...
import { EntityKind } from '@core/domain/kinds';
...
describe('entity traces', () => {
  ...
  it('records a trace and retrieves it', async () => {
    ...
    await repo.recordEntityTrace(EntityKind.Location, 'loc_burning_street', trace);
    const traces = await repo.getEntityTraces(EntityKind.Location, 'loc_burning_street', 10);
    expect(traces).toHaveLength(1);
    expect(traces[0]).toBe(trace);
  });

  it('returns traces oldest-first', async () => {
    await repo.recordEntityTrace(EntityKind.Location, 'loc_test', 'first');
    await repo.recordEntityTrace(EntityKind.Location, 'loc_test', 'second');
    const traces = await repo.getEntityTraces(EntityKind.Location, 'loc_test', 10);
  ...
  it('respects the limit (window) and returns only the most recent N', async () => {
    for (let i = 0; i < 5; i++) {
      await repo.recordEntityTrace(EntityKind.Location, 'loc_win', `effect-${i}`);
    }
    const traces = await repo.getEntityTraces(EntityKind.Location, 'loc_win', 3);
  ...
  it('scopes traces by world', async () => {
    await repo.recordEntityTrace(EntityKind.Location, 'loc_shared', 'world-a effect');
    const traces2 = await repo2.getEntityTraces(EntityKind.Location, 'loc_shared', 10);
